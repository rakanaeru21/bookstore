...
<head>
    ...
    <style>
        * {
        ...
        .current-cover-info h4 {
            color: #2c2416;
            margin-bottom: 4px;
        }

        .current-cover-info p {
            color: #6b5d4f;
            font-size: 14px;
        }
...
<body>
    <div class="container">
        <a href="{{ route('admin.buku.show', $buku->id_buku) }}" class="back-link">
        ...
        <div class="header">
            <h1>Edit Buku</h1>
            <p>Perbarui informasi buku "{{ $buku->Judul }}"</p>
        </div>

        <div class="form-container">
            <form method="POST" action="{{ route('admin.buku.update', $buku->id_buku) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <select id="id_kategori" name="id_kategori" class="form-select" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id_kategori }}"
                                {{ old('id_kategori', $buku->id_kategori) == $category->id_kategori ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    >{{ old('Sinopsis', $buku->Sinopsis) }}</textarea>
                    @error('Sinopsis')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($buku->Cover && \Illuminate\Support\Facades\Storage::disk('public')->exists($buku->Cover))
                        <div class="current-cover">
                            <img src="{{ \Illuminate\Support\Facades\Storage::url($buku->Cover) }}" alt="Cover saat ini">
                            <div class="current-cover-info">
                                <h4>Cover Saat Ini</h4>
                                <p>Pilih file baru untuk mengganti cover</p>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="file-upload">
                        ...
                        <label for="Cover" class="file-upload-label">
                            ...
                            <span>Pilih gambar cover baru atau drag & drop di sini</span>
                        </label>
                    </div>
                    ...
                    <small style="color: #6b5d4f; margin-top: 8px; display: block;">
                        Format: JPEG, PNG, JPG, GIF. Maksimal 2MB. Kosongkan jika tidak ingin mengubah cover.
                    </small>
                    @error('Cover')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('admin.buku.show', $buku->id_buku) }}" class="btn btn-secondary">
